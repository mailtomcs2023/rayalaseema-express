...
export function ShareBar({ title, slug, siteUrl, body }: { title: string; slug: string; siteUrl: string; body: string }) {
  const articleUrl = `${siteUrl}/article/${slug}`;
  const waText = `${title}\n\n${articleUrl}?utm_source=whatsapp\n\nRayalaseema Express లో చదవండి`;

  return (
    <div style={{ display: "flex", gap: 8, padding: "12px 0", borderBottom: "1px solid #eee", alignItems: "center", flexWrap: "wrap" }}>
      {/* WhatsApp - PRIMARY */}
      <a href={`https://wa.me/?text=${encodeURIComponent(waText)}`} target="_blank" rel="noopener noreferrer"
        style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "10px 20px", background: "#25D366", color: "#fff", borderRadius: 8, fontSize: 14, fontWeight: 800, textDecoration: "none", boxShadow: "0 2px 8px rgba(37,211,102,0.3)" }}>
      ...
      <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(articleUrl)}`} target="_blank" rel="noopener noreferrer"
        style={{ width: 34, height: 34, borderRadius: "50%", background: "#1877F2", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <a href={`https://t.me/share/url?url=${encodeURIComponent(articleUrl)}&text=${encodeURIComponent(title)}`} target="_blank" rel="noopener noreferrer"
        style={{ width: 34, height: 34, borderRadius: "50%", background: "#0088cc", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      {/* Copy */}
      <button onClick={() => { navigator.clipboard.writeText(articleUrl); alert("Link copied!"); }}
